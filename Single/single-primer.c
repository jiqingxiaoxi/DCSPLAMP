...
//caculate tm
int tm(char seq[],float deltah[],float deltas[],int length,float max_tm,float min_tm)
{
	int i,pos;
...
int stability(char seq[],float stab[],int length,int strand)
{
	int i,pos;
...
int candidate_primer(char *seq,char *prefix,char *dir,float stab[],float deltah[],float deltas[],int strand,float max_tm,float min_tm,int length,int P_flag,char *primer3_config)
{
	int i,circle,check,total,plus,minus;
	...
	if(P_flag)
	{
		fprintf(result,"PRIMER_TASK=check_primers\nPRIMER_PICK_ANYWAY=1\nPRIMER_SALT_DIVALENT=4\nPRIMER_DNTP_CONC=1.4\nPRIMER_DNA_CONC=38\n");
		fprintf(result,"PRIMER_THERMODYNAMIC_PARAMETERS_PATH=%s\n",primer3_config);
	}
	for(circle=0;circle<=(length-18);circle++)
	{
		for(i=18;i<=25;i++)  //read length is from 18 to 25
		{
			...
			if(check==0)
				continue;
                        ...
                        check=stability(primer,stab,i,strand);
                        if(check==1)     //+ strand
				plus=1;
			else
				plus=0;
			...
			check=stability(rev,stab,i,strand);
			if(check==1)
				minus=1;
			else
				minus=0;
		...
		//output
			if(plus+minus==0)
				continue;
			if(P_flag==0)
				fprintf(result,"pos:%d\tlength:%d\t+:%d\t-:%d\n",circle,i,plus,minus);
			else
			{
				if(plus==1)
                		{
                        		fprintf(result,"SEQUENCE_ID=%d-%d-+\n",circle,i);
                        		fprintf(result,"SEQUENCE_PRIMER=%s\n=\n",primer);
                		}
                		if(minus==1)
                		{
                        		fprintf(result,"SEQUENCE_ID=%d-%d--\n",circle,i);
...
int second_structure(char *prefix,char *dir,float tm_threshold,char *primer3)
{
	...
	len=strlen(dir)+strlen(prefix)+20;
	in=(char *)malloc(len);
	memset(in,'\0',len);
        strcpy(in,dir);
        strcat(in,prefix);
...
//check structure
	...
	minus=0;
	total=0;
	length=strlen(primer3)+200;
	line=(char *)malloc(length);
	memset(line,'\0',length);
        while(fgets(line,length,IN)!=NULL)
        {
...
{
	char *store_path,*prefix,*stab_path,*tm_path,*curren_path,*input,*outer,*inner,*loop,*primer3,*primer3_config;
	char *seq,*temp;
	FILE *fp;
	int i,flag[9],length,num_outer,num_inner,num_loop;
...
//input stability parameter
	...
	if(flag[5]==1)
	{
		num_outer=candidate_primer(seq,prefix,outer,stab,deltah,deltas,1,61,59,length,flag[8],primer3_config);
		num_inner=candidate_primer(seq,prefix,inner,stab,deltah,deltas,0,66,64,length,flag[8],primer3_config);
		if(flag[7]==1)
			num_loop=candidate_primer(seq,prefix,loop,stab,deltah,deltas,1,66,64,length,flag[8],primer3_config);
	}
	else
	{
		num_outer=candidate_primer(seq,prefix,outer,stab,deltah,deltas,1,56,54,length,flag[8],primer3_config);
		num_inner=candidate_primer(seq,prefix,inner,stab,deltah,deltas,0,61,59,length,flag[8],primer3_config);
                if(flag[7]==1)
                        num_loop=candidate_primer(seq,prefix,loop,stab,deltah,deltas,1,61,59,length,flag[8],primer3_config);
        }
	free(seq);
...
//check the second structure
	if(flag[5]==1)
	{
		num_outer=second_structure(prefix,outer,49,primer3);
		num_inner=second_structure(prefix,inner,54,primer3);
		if(flag[7]==1)
			num_loop=second_structure(prefix,loop,54,primer3);
	}
	else
	{
		num_outer=second_structure(prefix,outer,44,primer3);
                num_inner=second_structure(prefix,inner,49,primer3);
                if(flag[7]==1)
                        num_loop=second_structure(prefix,loop,49,primer3); 
        }
	//check
